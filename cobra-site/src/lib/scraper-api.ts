...
export interface ScraperLead {
  ...
  status: "WAITING_FOR_CALL" | "CALLED" | "BOUGHT" | string;
...
export async function getRuns(): Promise<ScrapeRun[]> {
  try {
    const res = await fetch(`${SCRAPER_API_URL}/runs`, { cache: "no-store" });
    if (!res.ok) return [];
    const data = await res.json();
...
export const MOCK_LEADS: ScraperLead[] = [
  ...
  },
  {
    ...
    twitter: null, linkedin: null, status: "BOUGHT", ai_summary: null,
