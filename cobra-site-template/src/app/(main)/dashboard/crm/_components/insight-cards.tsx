...
interface InsightCardsProps {
  categoryCounts?: { category: string; total: number }[];
}

export function InsightCards({ categoryCounts }: InsightCardsProps) {
  // Use real category counts if provided, otherwise fall back to config
  const chartData = categoryCounts
    ? categoryCounts.map((c, i) => ({
        source: c.category.toLowerCase().replace(/[^a-z]/g, ""),
        leads: c.total,
        fill: `var(--chart-${i + 1})`,
        label: c.category,
      }))
  ...
  const totalLeads = chartData.reduce((acc, curr) => acc + curr.leads, 0);

  // Build a config for the dynamic data
  const dynamicConfig = categoryCounts
    ? {
        leads: { label: "Leads" },
        ...Object.fromEntries(
          categoryCounts.map((c, i) => [
            c.category.toLowerCase().replace(/[^a-z]/g, ""),
            { label: c.category, color: `var(--chart-${i + 1})` },
          ]),
        ),
      }
    : leadsBySourceChartConfig;
    ...
    <div className="grid grid-cols-1 gap-4 *:data-[slot=card]:shadow-xs sm:grid-cols-2 xl:grid-cols-5">
      <Card className="col-span-1 xl:col-span-2">
        ...
        <CardContent className="flex items-center gap-6">
          <ChartContainer config={dynamicConfig} className="mx-auto aspect-square max-h-48 flex-1">
                    ...
                    if (viewBox && "cx" in viewBox && "cy" in viewBox) {
                      return (
                        <text x={viewBox.cx} y={viewBox.cy} textAnchor="middle" dominantBaseline="middle">
                          <tspan x={viewBox.cx} y={viewBox.cy} className="fill-foreground font-bold text-3xl tabular-nums">
                            {totalLeads}
                          </tspan>
              ...
              <li key={item.source} className="flex w-40 items-center justify-between">
                <span className="flex items-center gap-2 text-xs capitalize">
                  ...
                  {("label" in item ? item.label : null) ??
                    (dynamicConfig[item.source] as { label?: string } | undefined)?.label ??
                    item.source}
                </span>
        ...
        <CardFooter className="gap-2">
          <Button size="sm" variant="outline" className="basis-1/2" asChild>
            <a href="/dashboard/leads">View All Leads</a>
          </Button>
          <Button size="sm" variant="outline" className="basis-1/2" asChild>
            <a href="/dashboard/scrape">Run Scraper</a>
          </Button>
        </CardFooter>
      </Card>
      ...
      <Card className="col-span-1 xl:col-span-3">
        <CardHeader>
          <CardTitle>Leads by Category (Scraped vs. Target)</CardTitle>
        </CardHeader>
        <CardContent className="size-full max-h-52">
          <ChartContainer config={projectRevenueChartConfig} className="size-full">
            <BarChart accessibilityLayer data={projectRevenueChartData} layout="vertical">
              ...
              <YAxis dataKey="name" type="category" tickLine={false} tickMargin={10} axisLine={false} tickFormatter={(v) => v.slice(0, 4)} hide />
        ...
        <CardFooter>
          <p className="text-muted-foreground text-xs">Home Services dominates — 67% of all scraped leads</p>
        </CardFooter>
      </Card>
    </div>
  );
